...
RSpec.describe User, type: :model do
  context "validations" do
    ...
    describe "can be created when all attributes are present" do
      ...
      When(:user) { User.create(email: "testingfatcat@gmail.com", password: "123456") }
      Then { user.valid? == false }
    end


    describe "cannot be created without a email" do
      When(:user) { User.create(password: "123456", fullname: "Fatcat") }
      Then { user.valid? == false }
    end
    ...
    describe "should permit valid email only" do
      let(:user1) { User.create(fullname: "Fatcat", email: "testingfatcat@gmail.com", password: "123456")}
      let(:user2) { User.create(fullname: "Fatdog",email: "jack.com", password: "123456") }
